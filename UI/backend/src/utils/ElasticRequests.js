...
exports.simpleQuery = async (elasticClient, queryString, page, contentTypeFilter, filters, exactMatch) =>  {
    ...
    if(contentTypeFilter){
        queryBody["body"]["query"]["bool"]["filter"] = [
            { "match": { "contentType": contentTypeFilter }}]
    }
...
exports.simpleQuery = async (elasticClient, queryString, page, contentTypeFilter, filters, exactMatch) =>  {
    ...
    if(contentTypeFilter){
        console.log("Content Type Filter inside if statement");
...
exports.customQuery = async(elasticClient, index, body) => {
    return (await elasticClient.search({
        index,
        body
    }))
...
exports.bulkQuery = async(elasticClient, terms) => {
    const body = []
    terms.forEach(term => {
        body.push({ index: indexString})
        body.push({
            "size": 0,
            "query": {
                "bool": {
            ...
            "aggs": {
                [term]: { "terms": { "field": "contentType.keyword", "size": 50 } },
            }
        })
    });
    const response = await elasticClient.msearch({body});
    const returnObj = {}
    response.body.responses.forEach(resp => {
        const key = Object.keys(resp.aggregations)[0]
        returnObj[key] = resp.aggregations[key].buckets
    })
    return returnObj;
}
...
function applyFiltersToQuery(queryBody, filters){
    if(filters){
        ...
        // if(filters.filteredExhibits && filters.filteredExhibits.length > 0){
        //     queryBody["body"]["query"]["bool"]["must"].push({"terms": {"exhibit.keyword": filters.filteredExhibits}})
        // }

        // if(filters.filteredOperations && filters.filteredOperations.length > 0){
        //     queryBody["body"]["query"]["bool"]["must"].push({"terms": {"operation.keyword": filters.filteredOperations}});
        // }

        //COMMENT: This should be able to go through each and every filter we could add for checkbox values
        ...
        //         const keyValue = {};
        //         keyValue[item.name] = item.values[0];
        //         queryBody["body"]["query"]["bool"]["should"].push({"match": keyValue});
        //     });
        // }
        ...
        //             const keyValue = {};
        //             keyValue[item.name] = itemValue;
        //             queryBody["body"]["query"]["bool"]["must"].push({"match": keyValue});
        //         });
        //     });
        // }
        ...
        if(filters.allDates == false){
            console.log("WITHIN DATE FILTERS");
            console.log(filters);
            const rangeFilter = {"range": {"coverage_start_date": { "gte": filters.startDate, "lte": filters.endDate}}};
            console.log(rangeFilter);
            console.log( queryBody["body"]["query"]);
            console.log( queryBody["body"]["query"]["bool"]);
